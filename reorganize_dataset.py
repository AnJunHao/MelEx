...
def reorganize_dataset():
    ...
    for row in sheet.iter_rows(min_row=2, max_col=10, values_only=True):
        if row[0] is None:
            break
        table.append(Row(*[str(i) for i in row[:4]]))
    ...
    for row in table:
        ...
        file_mappings = [
            ...
            (base_path / "melody" / f"{row.hash_}.mid", f"{song_name}.m.mid"),
            (base_path / "piano_melody_test_samples" / row.id_ / f"{row.id_}_transcription.mid", f"{song_name}.t.mid"),
            (base_path / "piano_melody_test_samples" / row.id_ / f"{row.id_}_rectified.mid", f"{song_name}.gt.mid"),
            (base_path / "piano_melody_test_samples" / row.id_ / f"{row.id_}.mid", f"{song_name}.bl.mid"),
        ]
    ...
    with open(summary_file, 'w', encoding='utf-8') as f:
        ...
        f.write("- {song_name}.note     - Melody annotations\n")
